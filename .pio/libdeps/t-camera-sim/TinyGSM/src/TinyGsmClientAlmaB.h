...
#ifndef SRC_TINYGSMCLIENT_ALMAB_H_
#define SRC_TINYGSMCLIENT_ALMAB_H_
...
class TinyGsmAlmaB : public TinyGsmModem<TinyGsmAlmaB>,
                     public TinyGsmGPRS<TinyGsmAlmaB>,
                     public TinyGsmUDP<TinyGsmAlmaB, TINY_GSM_MUX_COUNT>,
                     public TinyGsmSMS<TinyGsmAlmaB>,
                     public TinyGsmGSMLocation<TinyGsmAlmaB>,
                     public TinyGsmGPS<TinyGsmAlmaB>,
                     public TinyGsmTime<TinyGsmAlmaB>,
                     public TinyGsmNTP<TinyGsmAlmaB>,
                     public TinyGsmBattery<TinyGsmAlmaB>,
                     public TinyGsmTemperature<TinyGsmAlmaB>,
                     public TinyGsmCalling<TinyGsmAlmaB>
{
  friend class TinyGsmModem<TinyGsmAlmaB>;
  friend class TinyGsmGPRS<TinyGsmAlmaB>;
  friend class TinyGsmUDP<TinyGsmAlmaB, TINY_GSM_MUX_COUNT>;
  friend class TinyGsmSMS<TinyGsmAlmaB>;
  friend class TinyGsmGPS<TinyGsmAlmaB>;
  friend class TinyGsmGSMLocation<TinyGsmAlmaB>;
  friend class TinyGsmTime<TinyGsmAlmaB>;
  friend class TinyGsmNTP<TinyGsmAlmaB>;
  friend class TinyGsmBattery<TinyGsmAlmaB>;
  friend class TinyGsmTemperature<TinyGsmAlmaB>;
  friend class TinyGsmCalling<TinyGsmAlmaB>;

  /*
   * Inner Client
   */
public:
  class GsmClientAlmaB : public GsmClient
  {
    friend class TinyGsmAlmaB;

  public:
    GsmClientAlmaB() {}

    explicit GsmClientAlmaB(TinyGsmAlmaB &modem, uint8_t mux = 0)
    {
      init(&modem, mux);
    }

    bool init(TinyGsmAlmaB *modem, uint8_t mux = 0)
    {
      this->at = modem;
      sock_available = 0;
      prev_check = 0;
  ...
  /*TODO(?))
  class GsmClientSecureAlmaB : public GsmClientAlmaB
  {
  public:
    GsmClientSecure() {}

    GsmClientSecure(TinyGsmAlmaB& modem, uint8_t mux = 0)
  ...
  */
   ...
   */
public:
  explicit TinyGsmAlmaB(Stream &stream) : stream(stream)
  {
...
protected:
  bool initImpl(const char *pin = NULL)
  {
    ...
    DBG(GF("### TinyGSM Compiled Module:  TinyGsmClientAlmaB"));
...
protected:
                       ...
                       const char *pwd = NULL)
  {
    ...
    // Select UDP/IP application mode (command mode)
    sendAT(GF("+CIPMODE=0"));
    waitResponse();

    // Set Sending Mode - send without waiting for peer UDP ACK
    ...
    // to the embedded context for UDP/IP (ie AT+CGACT=1,1 and AT+CGATT=1)
...
protected:
                    ...
                    bool ssl = false, int timeout_s = 15)
  {
    ...
    // AT+CIPOPEN=<id>,"UDP","<remote_ip>",<remote_port>,<local_port>
    // AT+CIPOPEN=1,\"UDP\",,,PORT
    sendAT(GF("+CIPOPEN="), mux, ',', GF("\"UDP"), GF("\",\"\","), 0, ',', port);
  ...
  uint16_t modemWrite(const void *buff, size_t len)
  {
    stream.write(reinterpret_cast<const uint8_t *>(buff), len);
    return len;
  }

  bool modemBeginPacket(size_t len, uint8_t mux, const char *host, uint16_t port)
  {
    sendAT(GF("+CIPSEND="), mux, ',', (uint16_t)len, ',', GF("\""), host, GF("\""), ',', port);
...
protected:
  GsmClientAlmaB *sockets[TINY_GSM_MUX_COUNT];
  const char *gsmNL = GSM_NL;
};

#endif // SRC_TINYGSMCLIENT_ALMAB_H_
